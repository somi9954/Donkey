...
import org.project.donkey.models.board.*;
...
public class BoardController {
    ...
    public String write(@PathVariable("bId") String bId, @ModelAttribute BoardForm form, Model model) {
        ...
        if (customJwtFilter.isUserLoggedIn()) {
            form.setPoster(customJwtFilter.getEntity().getUserNm());
        }
    ...
    public String update(@PathVariable("seq") Long seq, Model model) {
        if (!infoService.isMine(seq)) { // 직접 작성한 게시글이 아닌 경우
            throw new BadRequestException(Utils.getMessage("작성한_게시글만_수정할_수_있습니다.", "error"));
        }
    ...
    public String delete(@PathVariable("seq") Long seq) {
        if (!infoService.isMine(seq)) {
            throw new BadRequestException(Utils.getMessage("작성한_게시글만_삭제_가능합니다.", "error"));
        }
    ...
    private void commonProcess(String bId, String mode, Model model) {
        ...
        if (board == null || (!board.isActive() && !customJwtFilter.isUserAdmin())) { // 등록되지 않거나 또는 미사용 중 게시판
        ...
        if (mode.equals("write") || mode.equals("update")) {
            ...
            if (!customJwtFilter.isUserAdmin() && !customJwtFilter.isUserLoggedIn()
                    && authority == BoardAuthority.MEMBER) { // 회원 전용
                throw new BadRequestException(Utils.getMessage("MemberOnly.board", "error"));
            }

            if (authority == BoardAuthority.ADMIN && !customJwtFilter.isUserAdmin()) { // 관리자 전용
                throw new BadRequestException(Utils.getMessage("AdminOnly.board", "error"));
            }
        }
        /* 글쓰기, 수정시 권한 체크 E */
