...
import org.project.donkey.commons.exceptions.AlertBackException;
import org.project.donkey.commons.exceptions.BadRequestException;
import org.project.donkey.configs.jwt.CustomJwtFilter;
import org.project.donkey.controllers.comments.CommentForm;
...
public class CommentInfoService {
    ...
    public void isMine(Long seq) {
        if (customJwtFilter.isUserAdmin()) {
            return;
        }
        ...
        if (commentMember == null) { // 비회원 작성
            ...
            if (!customJwtFilter.isUserLoggedIn()
                    || commentMember.getUserNo().longValue() != customJwtFilter.getEntity().getUserNo().longValue()) {
    ...
    public boolean isEditable(CommentData comment) {
        ...
        if (customJwtFilter.isUserAdmin() || commentMember == null) { // 관리자이거나 비회원 댓글이면 무조건 노출
            return true;
        }

        // 회원 댓글이면 직접 작성한 댓글만 노출
        if (customJwtFilter.isUserLoggedIn() && commentMember != null
                && commentMember.getUserNo().longValue() == customJwtFilter.getEntity().getUserNo().longValue()) {
            return true;
        }
